...
// ── Sprite cache (module-level, shared across cards) ─────────────────────────
const spriteCache = new Map<number, PokemonSprite>()

// ── Slot component ────────────────────────────────────────────────────────────

function MemberSlot({
  position, member, token, teamId,
  onAdd, onRemoved, onMemberUpdated,
}: {
  position:         number
  ...
  const saveNick = async () => {
    ...
    setSavingNick(true)
    try {
      const updated = await updateMember(token, teamId, member.id, { nickname: nickVal.trim() })
  ...
  const handleRemove = async () => {
    ...
    setRemoving(true)
    try {
      await removeMember(token, teamId, member.id)
      onRemoved(member.id)
        ...
        {editNick ? (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Box
              ...
              onKeyDown={(e: React.KeyboardEvent) => { if (e.key === 'Enter') saveNick(); if (e.key === 'Escape') setEditNick(false) }}
              maxLength={24}
              sx={{
                flex: 1, minWidth: 0,
          ...
          <Box
            ...
            onClick={() => { setNickVal(member.nickname); setEditNick(true) }}
          >
            <Typography sx={{
...
export default function TeamCard({ team, token, onPickPokemon, onTeamUpdated, onTeamDeleted }: Props) {
  ...
  const slots = Array.from({ length: 6 }, (_, i) => {
    const pos = i + 1
    return team.members.find(m => m.position === pos) ?? null
  })
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2.5, pt: 2, pb: 1.5 }}>
        ...
        <Box
          ...
          sx={{
            color: team.favorite ? '#f8d030' : '#3d0000',
            cursor: 'pointer', lineHeight: 0, flexShrink: 0,
            transition: 'color 0.15s, transform 0.15s',
            '&:hover': { color: '#f8d030', transform: 'scale(1.2)' },
          }}
        >
          {togglingFav
            ? <CircularProgress size={14} sx={{ color: '#f8d030' }} />
            : <StarIcon filled={team.favorite} />}
        </Box>
          ...
          {editName ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              <Box
                ...
                sx={{
                  flex: 1, minWidth: 0,
                  fontFamily: PIXEL, fontSize: '11px', color: '#f8d030',
                  background: '#0d0000', border: '1.5px solid #cc0000',
                  borderRadius: '4px', px: '8px', py: '4px', outline: 'none',
            ...
            <Box
              onClick={() => { setNameVal(team.name); setEditName(true) }}
              sx={{ display: 'flex', alignItems: 'center', gap: 0.75, cursor: 'pointer', '&:hover .edit-icon': { opacity: 1 } }}
            >
              <Typography sx={{
                fontFamily: PIXEL, fontSize: '12px', color: '#f8d030',
                letterSpacing: '0.08em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                textShadow: team.favorite ? '0 0 10px rgba(248,208,48,0.5)' : 'none',
              }}>
                {team.name}
              </Typography>
        ...
        <Box
          ...
          sx={{
            color: confirmDel ? '#ff4444' : '#3d1010',
            cursor: 'pointer', lineHeight: 0, flexShrink: 0,
      ...
      {/* Meta row */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 2.5, pb: 2 }}>
        ...
        {confirmDel && (
          <Typography
            ...
            sx={{ fontFamily: PIXEL, fontSize: '8px', color: '#ff4444', letterSpacing: '0.06em', cursor: 'pointer', ml: 'auto' }}
          >
      ...
      {/* 6-slot grid */}
      <Box sx={{ px: 2.5, pb: 2.5, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5, '@media (min-width:480px)': { gridTemplateColumns: 'repeat(6, 1fr)' } }}>
        {slots.map((member, i) => (
          <MemberSlot
            key={i}
            position={i + 1}
            ...
            onRemoved={(memberId) => {
              onTeamUpdated({ ...team, members: team.members.filter(m => m.id !== memberId) })
            }}
            onMemberUpdated={(updated) => {
              onTeamUpdated({ ...team, members: team.members.map(m => m.id === updated.id ? updated : m) })
            }}
          />
        ))}
      </Box>
